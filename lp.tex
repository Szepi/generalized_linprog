...
\end{split}
\end{align}
Unless specified otherwise we use $\tr$ to denote any solution\todoc{instead of saying the solution} to the ALP and $\tj=\Phi \tr$ to denote the corresponding approximate value function. We now state the assumptions and definitions used for the rest of the paper.
...
The weighted $L_1$-norm is defined as \todoc{modified $\Rightarrow$ weighted?}
\begin{align*}
...
The weighted $L_\infty$-norm is defined as follows:  \todoc{modified $\Rightarrow$ weighted? Also, the convention in math is to divide in the case of the $L_{\infty}$ norm. Not a high priority to change this, but maybe it would be good to do this in the longer term.}
\begin{align*}
