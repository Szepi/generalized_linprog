...
In fact, it is not hard to observe that $(\Gamma J)(i) = (\Phi r_{e_i})(i)$ for any $1\le i \le n$; the reason $\Gamma$ is defined the way it is so that this resembles to \eqref{alp} will be clear. \todoc{In fact, I would prefer the simpler definition without the $\min$ and then changing this remark.}
\end{enumerate}
\end{remark}
...
\todoc{Poor choice of symbols: $\tilde{V}$ is the fixed point of $\Gamma$ and $\hat{V}$ will be the fixed point of $\tilde{\Gamma}$? Hmm.. Not a high priority to change it, but still..}
