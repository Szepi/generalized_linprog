...
\todoc{This is somewhat limited; normally we should allow history dependent,
...
\todoc{Probably better to define first $J^*$ as the pointwise max and then the optimal policy's definition could use $J^*$.}
Given an MDP, our aim is to find the optimal value function $J^*$ and the optimal policy SDP $u^*$. The optimal policy and value function obey the Bellman equation (BE): for all $ s \in S$, \todoc{Non-unique maximizers in the policy def?}
