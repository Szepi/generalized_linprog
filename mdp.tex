...
The infinite horizon discounted reward corresponding to state $s$ under a policy $\pi$ is denoted by $J_\pi(s)$ and is defined by \todoc{Explain the meaning of $\eqdef$.}
\begin{align}
J_\pi(s) \eqdef
...
Here $J_\pi(s)$ is known as the value of the state $s$ under the policy $\pi$, and $J_\pi:S\to \R$ is 
 %the vector quantity $J_\pi\eqdef(J_\pi(s))_{s\in S}\in R^n$ is 
...
\todoc{Introduce $\norm{\cdot}_{\infty}$. Should we have a notation/symbols section?}
\begin{align}
