...
For example, \citet{schuurmans} and \citet{gkp} propose constraint generation in a setting 
where the MDP has additional structure (i.e., factorized transition structure) 
that is then exploited in the constraint generation method.
A more general approach due to \citet{CS} 
...
While constraint generation is powerful when applicable,
