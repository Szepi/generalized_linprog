...
We now define the two projection operators which are central to our error analysis. The least upper bound (LUB) projection operator $\Gamma \colon \R^n \ra\R^n$ is defined below, see \eqref{gamdef}.
...
\end{align}
where $J\in \R^n$. Thus $\F_J$ contains vectors in the span of $\Phi$ that upper bound $TJ$. Further, since $(\Gamma J)(i) = \min\{ V(i) \,:\, V\in \F_J \}$, it also follows that $ V\ge \Gamma J $ holds for any $V\in \F_J$.
\end{remark}
...
\end{lemma}
\begin{proof}
We know from Lemma~\ref{relation2} that $\tj = \Phi \tr \geq\hv$, and the rest of the arguments follow in the same manner as proof for \cref{srw}.
\end{proof}
\begin{lemma}\label{cmt1mn}
We have
\begin{align}
||J^*-\hv||_{\mn}&\leq \frac{1}{{1-\beta_{\psi}}}\big(2||J^*-\Phi r^*||_{\mn}&\nn\\+||\Gamma J^*-\hg J^*||_{\mn}\big).
\end{align}
\end{lemma}
\begin{proof}
...
For this result, recall that $r_0 \in \R^k$ is the vector such that $\psi = \Phi r_0$ by assumption.
...
&||\Phi r'-J^*||_{\mn}\nn\\&\leq ||\Phi r^*-J^*||_{\mn}+||\Phi r'-\Phi r^*||_{\mn}\nn\\
&=\gamma+\frac{1+\beta_\psi}{1-\beta_\psi}\gamma\nn\\
&=\frac{2}{1-\beta_\psi}\gamma.
\end{align}
%We know that $\tr \in \N$ by \cref{grlpassmp}-\eqref{nassmp} and 
$r'$ is feasible for the ALP in \eqref{grlp} by \cref{restate}. Then from \cref{srwalp} it follows that
%As a result, from \cref{srw} it follows that
\begin{align}
&||\hj-\hv||_{1,c}\leq||\Phi \tr-\hv||_{1,c}\leq||\Phi r'-\hv||_{1,c}\nn\\
...
&||J_{\hu}-\hj||_{1,c}\nn\\&=||(I-\alpha P_{\hu})^{-1}(T\hj-\hj)||_{1,c}\nn\\
