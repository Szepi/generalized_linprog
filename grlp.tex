...
where the choice of $m$ and that of the vectors $w_{i,a}\in [0,\infty)^S$ is left to the user.
...
\end{split}
\end{align}
where $W_a = (w_{1,a},\dots,w_{m,a}) \in [0,\infty)^{S \times m}$. 
...
We will call the LP in \eqref{grlp} the \emph{linearly relaxed approximate linear program (LRALP)}. \todoc{Replace GRLP everywhere with LRALP.} Note that the LRALP may be unbounded. While unboundedness could be prevented by adding an extra constraint of the form $r\in \N$ for a ``sufficiently large'' and ``simple'' bounded polytope $\N \subset \R^k$ which can be chosen based on the knowledge of $\alpha$ alone. However, we see little value in this and thus stick to the above simple form.%
\footnote{
Note though that $\N$ would need to be chosen based on very little information,
hence it would in general be chosen to provide an extremely loose constraint to avoid
the accidental elimination of potentially good solutions. 
Hence, on the occasion when the LP without the extra $r\in \N$ constraint is unbounded,
...
Since the resulting program may become unbounded,  it is necessary to introduce an additional constraint $r\in \N$, where $\N$ will be chosen to be large enough so as not to exclude ``good solutions'', while constraining enough so as to prevent unbounded solutions. 
