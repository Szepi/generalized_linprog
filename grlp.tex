...
The operators $\Gamma$ and $\hg$ are closely related to ALP \eqref{alp} and GRLP \eqref{grlp} respectively and are only analytical tools that we will need to express our bounds and need not be computed in practice. It turns out that the novel operators ($\Gamma$ and $\hg$) are contraction maps (see \Cref{hgmaxcontramn}), a fact that is a key to our results (\Cref{cmt2mn,polthe}). Though we are concerned about the behaviour of $\Gamma$ and $\hg$ on the candidate value functions in the constraint set $\N$, we want them to make sense for any arbitrary $J\in \R^n$. In order to be accomplish the same, we define $\N'$ in \Cref{lubplp,alubplp} to be `$\N$ plus its translations by $\one$', so as to patch $\N$ via translations and make the neighbourhood of any arbitrary $J\in \R^n$, `look' like $\N$.
\end{remark}
