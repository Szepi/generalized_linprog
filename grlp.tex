...
\end{split}
\end{align}
where $W \in \R_+^{nd\times m}$ is an $nd\times m$ matrix with all nonnegative entries and $\N \subset \R^k$ is any bounded set such that $\tilde{r}_c \in \N$. \todoc{Is not $\hj\in \R^n$?
Moreover, later often $\chi$ is not taken into account. 
Also, the role of $\chi$ is never explained. Do we want to keep $\chi$ or maybe we should just remove it?
Further, $\hj$ is not really well defined. We need to defined $r_c$ and then set $\hj = \Phi r_c$.
}
